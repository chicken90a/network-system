...
backup_file_restore = "mikrotik-cm-bk-1810.backup"
backup_file = f"mikrotik-cm-bk-{time.strftime('%Y%m%d')}"
password_backup_file = "123456"

#/system backup save name=my-backup password=myPassword
...
def mikrotik_backup(host, username, password, backup_file):
    try:
        ...
        command = f"/system backup save name={backup_file} password={password_backup_file}"
...
def restore_backup(host, username, password, backup_file_restore):
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())

    print(f"Connecting to {host} to restore backup...")
    client.connect(host, username=username, password=password)
    restore_command = f"/system backup load name={backup_file_restore} password=123456" #f"/import file={os.path.basename(backup_file_restore)}" #f"/system backup load name={os.path.basename(backup_file)} password={password}"
    # sftp = client.open_sftp()
    # sftp.put(backup_file, os.path.basename(backup_file_restore))
    # sftp.close()
...
if not modified_lines and not added_line:
    ...
    #restore_backup(host, username, password, backup_file_restore)
else:
    raise Exception("Configuration changes detected. Stopping execution.")
#restore_backup(host, username, password, backup_file_restore)