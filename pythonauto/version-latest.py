...
def check_remove_command(command):
    ...
    if parts[2] == "address-list" and "add" in parts:
        ...
        if address and list_name:
            command = f"/ip firewall address-list remove [find {address} {list_name}]"
    ...
    #mangle delete
        ...
        if comment_full:
            #command = f"/ip firewall mangle remove [find action=accept chain=prerouting {comment_full} {dst_address_list} {src_address}]"
            command = f"/ip firewall mangle remove [find {comment_full}]"
            return command
    ...
    #nat delete
        ...
        to_address = next((part for part in parts if part.startswith("to-addresses=")), None)
        print(to_address)

        to_port = next((part for part in parts if part.startswith("to-ports=")), None)
        print(to_port)

        if dst_address and dst_port and to_address and to_port:
            #command = f"/ip firewall nat remove [find action=dst-nat chain=dstnat {dst_address} {dst_port} protocol=tcp {to_address} {to_port}"
            command = f"/ip firewall nat remove [/ip firewall nat find {to_address}]"
            return command
        ...
        if comment:
            full_comment = comment
            for part in parts[parts.index(comment) + 1:]:
                if part.startswith("dst-address-list=") or part.startswith("dst-address="):
                    break
                full_comment += ' ' + part
                comment_full = full_comment.strip()
            print(comment_full)
        if comment_full:
            command = f"/ip firewall filter remove [find {comment_full}]"
            return command

    #/ip firewall filter remove [find comment="Test changerequest tpm"]
